...
// Greitas kelias iš LIVE lango: pirma pasirenkamas turnyras (kad transliacija
// teisingai prisikabintų), tada pereinama į kameros skirtuką ir pradedama transliacija.
...
function rtcChooseTournament(room) {
    ...
    if (room) {
        ...
        if (typeof connectLiveRoom === 'function') connectLiveRoom();   // prisijungia (tablo + žyma)
        else if (typeof currentLiveRoomName !== 'undefined') currentLiveRoomName = String(room).toUpperCase();
    } else {
        if (typeof currentLiveRoomName !== 'undefined') currentLiveRoomName = null;  // bendra → kris į profilį
    }
    rtcGoToCameraAndBroadcast();
}

function rtcGoToCameraAndBroadcast() {
...
// Žiūrovo nuoroda: atidarius ją, transliacija paleidžiama automatiškai (rtcHandleWatchParam).
// Privačiai transliacijai PIN įdedamas į nuorodą — QR skenuoja tik esantys prie korto.
function rtcViewerLink() {
    let url = location.origin + location.pathname + '?watch=' + encodeURIComponent(rtcBroadcastId);
    if (rtcBroadcastPin) url += '&pin=' + encodeURIComponent(rtcBroadcastPin);
    return url;
}

function rtcQrSvg(url) {
    try {
